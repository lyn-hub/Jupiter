...
public class MySQLConnection {
    ...
    public MySQLConnection() throws MySQLException {
        try { // in case it failed: reflect (only when run this line, then can determine the behavior)
            Class.forName("com.mysql.cj.jdbc.Driver").newInstance();
            //creat the connection with AWS mysql
            conn = DriverManager.getConnection(MySQLDBUtil.getMySQLAddress());
        } catch (Exception e) { // if failed, throw exception
            e.printStackTrace();
    ...
    public void close() {
        if (conn != null) {
            try {
                conn.close();  //disconnected the connection, otherwise will occupied the port
                               // every time, you open the connection, it will use the memory
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public void setFavoriteItem(String userId, Item item) throws MySQLException {
        ...
        saveItem(item);
        //if have duplicate insert, just ignore it, because it will not change the db;
